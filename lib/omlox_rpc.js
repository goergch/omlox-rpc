...
class OmloxRPC extends EventEmitter {
  constructor(address) {
    super();
    this.connectionState = OmloxRPC.CONNECTION_STATE.OFF;
  ...
  connect() {
    ...
    this.setConnectionstate(OmloxRPC.CONNECTION_STATE.CONNECTING);
    this.client.connect(this.address);
  }

  setConnectionstate(conState) {
    if (conState !== this.connectionState) {
      const oldState = this.connectionState;
      this.connectionState = conState;
      this.emit('connectionStateChanged', { oldState, newState: this.connectionState });
    }
  }
  ...
  onWebsocketConnect(connection) {
    ...
    this.setConnectionstate(OmloxRPC.CONNECTION_STATE.CONNECTED);
  }

  onWebsocketConnectFailed(errorDescription) {
    this.emit('wsConnected', false);
    this.setConnectionstate(OmloxRPC.CONNECTION_STATE.ERROR);
    logger.info('OmloxRPC connection failed:', errorDescription);
    this.connection = null;
    setTimeout(() => {
      logger.info('OmloxRPC reconnecting');
      this.client.connect(this.address);
    }, 10000);
  }
  ...
  onClose(reasonCode, description) {
    this.methods = {};
    this.emit('wsConnected', false);
    this.setConnectionstate(OmloxRPC.CONNECTION_STATE.OFF);
