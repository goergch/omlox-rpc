...
class OmloxRPC extends EventEmitter{
    ...
    onWebsocketConnect(connection){
        ...
        if(this.methods != undefined){
            async.each(this.methods,(method, cb)=>{
                if(method.zoneid !== undefined){
                    registerMethod(method.methodname, connection, method.zoneid)
                    .then(()=>{
                        logger.info("registered method " + method);
                        cb();
                    })
                    .catch((err)=>{
                        logger.error("Error on registering method " + method + ": " + err);
                        cb(err, method);
                    })
                } else{
                    registerMethod(method.methodname, connection)
                    .then(()=>{
                        logger.info("registered method " + method);
                        cb();
                    })
                ...
                logger.info("All method registrations done")
                this.emit('registrationdone', results);
            })
        }
    ...
    onMessage(message){
        ...
        if (msgObj.jsonrpc && msgObj.jsonrpc === '2.0' && msgObj.method !== undefined){
            if(this.methods.includes(msgObj.method) ){
                //handle method
                var result = "This is the result for " + msgObj.method;
